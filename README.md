...
1.  **State Machine Operation:** Every task rigorously follows the `Discovery` -> `Strategy` -> `Execution` -> `Reflection` cycle.
...
*   **🧠 Advanced System Prompt:** The `ELITE_PROMPT.md` file is structured using the official `<role_and_goal>`, `<instructions>`, `<constraints>`, and `<definitions>` format for maximum model alignment.
...
*   **`/elite:audit` (🧊 Audit Mode):** Forces an immediate stop and entry to the **Reflection** state for a deep quality check.
*   **`/elite:boot` (🔄 System Reboot):** Forces a complete re-read of the system prompt and clears working memory.
*   **`/elite:design` (📐 Design Mode):** Enters the **Strategy** phase. Reads the master backlog and integrates your new request into a revised plan.
*   **`/elite:log` (💾 Log State):** Saves the current proposed plan from the agent's memory to the master backlog file at `.gemini/BACKLOG.md`.
*   **`/elite:reason` (⚖️ Reasoning Mode):** Pauses execution to use first-principles thinking to overcome a blocker or explore a new architectural approach.
...
3.  **Trust the Cycle:** Allow the agent to follow its `Discovery` -> `Strategy` -> `Execution` -> `Reflection` loop. Read its `<reflection>` block to understand its thinking.
